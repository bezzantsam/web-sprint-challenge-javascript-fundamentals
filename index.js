...
//Nested can access internal because the lexical scope of Javascript states that functions can reach outside of themselves for a variable. It doesn't work the opposite way. So internal is outside of nested in scope so it is accessible.
...
function lowerCaseNames(array) {
  const lowerCased = array.map((item) => item.animal_name.toLowerCase());
  return lowerCased;
}
...
function lowPopulationAnimals(array) {
  const result = array.filter((item) => item.population < 5);
  return result;
}
